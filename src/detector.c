	...
	float hier_thresh, int dont_show, int ext_output, int save_labels, char *rect_txt)
{
	...
	for (int i = 0; i < num; i++){
        ...
        image im = load_image(img_files[i],0,0,net.c);
		//printf("img_file=%s\n", img_files[i]);
		//printf("net.c = %d\n", net.c);
        int letterbox = 0;
        ...
        printf("%s: Predicted in %f seconds.\n", img_files[i], (what_time_is_it_now()-time));
		...
		int s = strlen(img_files[i]);
		char step_i1[5] = { img_files[i][s - 13], img_files[i][s - 12], img_files[i][s - 11], img_files[i][s - 10], '\0' };
		char step_j1[5] = { img_files[i][s - 8], img_files[i][s - 7], img_files[i][s - 6], img_files[i][s - 5], '\0' };
		int step_i = atoi(step_i1);
		int step_j = atoi(step_j1);

		draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output, left, right, top, bot, step_i, step_j, rect_txt);
		...
		//draw_detections(im, dets, thresh, nboxes, names, alphabet, l.classes);
        ...
        {
            char labelpath[4096];
            replace_image_to_label(img_files[i], labelpath);
...
void run_detector(char * datacfg, char *cfg, char *weights, char *filename, float thresh, char *rect_txt)
{
	...
	fp = fopen(rect_txt, "w");
	fclose(fp);
	test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, rect_txt);
}
